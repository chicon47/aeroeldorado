    ...
    <script>
        $(function() {
            ...
            function carregaMatriculas() {
                ...
                $.ajax({
                    type: 'ajax',
                    url: '<?php echo base_url() ?>aeronaves/carregaAcfts',
                    async: false,
                    dataType: 'json',
                    success: function(data) {
                        ...
                        for (i = 0; i < data.length; i++) {
                                    ...
                                    html += '<tr>' +
                                            '<td hidden>' + data[i].cod_aeronave + '</td>' +
                                            '<td>' + data[i].modelo_aeronave + '</td>' +
                                            '<td>' + data[i].nro_aeronaves + '</td>' +
                                            '<td>' +
                                            '<a href="javascript:;" class="glyphicon glyphicon-pencil edit-acft" data="' + data[i].cod_aeronave + '"></a>' +
                                            '</td>' +
            ...
            $('#showdata').on('click', '.edit-acft', function() {
